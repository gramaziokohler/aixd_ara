...
def plot_browser():
    ...
    blocks = []
    attributes = ['sun_total', 'rain_total', 'surface_total']

    if plot_type == "distrib_attributes":
        fig = plotter.distrib_attributes(
            blocks=blocks, attributes=attributes, per_column=True, bottom_top=(0.1, 0.9), downsamp=1)
    elif plot_type == "distrib_attributes2d":
        fig = plotter.distrib_attributes2d(
            blocks=blocks, attributes=attributes)
    elif plot_type == "contours2d":
        fig = plotter.contours2d(blocks=blocks, attributes=attributes)
    elif plot_type == "correlation":
        fig = plotter.correlation(blocks=blocks, attributes=attributes)
    else:
