...
def DataInt_from_shallow(shallow_dobj):
    return DataInt(name=shallow_dobj["name"], dim=shallow_dobj["dim"], domain=Options(shallow_dobj["domain"]) if shallow_dobj["domain"] else None)


def DataReal_from_shallow(shallow_dobj):
    return DataReal(name=shallow_dobj["name"], dim=shallow_dobj["dim"], domain=Interval(*shallow_dobj["domain"]) if shallow_dobj["domain"] else None)


def DataCategorical_from_shallow(shallow_dobj):
    return DataCategorical(name=shallow_dobj["name"], dim=shallow_dobj["dim"], domain=Options(shallow_dobj["domain"]) if shallow_dobj["domain"] else None)
...
def dataobjects_from_shallow(shallow_dobjs: List[Dict]):  # -> List[DataObject]:
    ...
    for sh_dobj in shallow_dobjs:
        if sh_dobj["datatype"] == "DataBool":
            ...
            raise ValueError(f"Unknow data type {sh_dobj['datatype']} in a shallow data object. Allowed values (string) are: 'DataBool', 'DataCategorical', 'DataInt', 'DataReal'.")
